...
export const hyperdriveHttpGateway = function hyperdriveHttpGatewayFactory ({ client, corestore }) {
  ...
  return async function hyperdriveHttpGateway (req, res, next) {
    try {
      ...
      const content = await fetchHyperdriveFile(client, corestore, driveInfo)
...
export async function fetchHyperdriveFile (client, corestore, { publicKey, filePath }) {
